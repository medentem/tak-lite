...
## Refactor Plan: Prefer Device Location for User Position

This plan ensures the app always prefers the Meshtastic device's location when connected, and that this logic is consistent with the approach already used for Layer 2 devices. The goal is a single, reliable "current user location" source, regardless of mesh protocol.

### 1. Analyze Existing Layer 2 Handling
- Layer 2 (MeshNetworkProtocol) already prefers the device's location if available, falling back to the phone's location only if the device is not connected or not providing updates.
- There is likely a central place (e.g., a StateFlow or similar) where the "current user location" is exposed to the UI and other app features.

### 2. Identify Where Meshtastic Location Is Handled
- MeshtasticBluetoothProtocol receives position packets in `handleIncomingPacket`.
- The app currently may update peer locations, but may not update the "current user location" for the app/UI.
- The phone's location may still be used for the user's own marker, even when a device is connected.

### 3. Refactor Plan
#### A. Centralize "Current User Location" State
- Create or update a single StateFlow (or similar) in MeshNetworkService to represent the user's current location.
- This StateFlow should be updated by both protocol types (Layer 2 and Meshtastic).
- The UI and all app features should observe this StateFlow for the user's location.

#### B. Update MeshtasticBluetoothProtocol to Emit User Location
- In `handleIncomingPacket`, when a position packet is received from the connected device and it matches the user's own node ID:
  - Update the centralized "current user location" StateFlow.
- Ensure that only the device's own position packets (not those from other peers) are used for this.

#### C. Fallback Logic
- If the device is not connected, or no recent position packet has been received, fallback to the phone's GPS.
- Implement a timeout/staleness check (e.g., if no device update in 2-3 minutes, fallback to phone).

#### D. UI/UX Consistency
- Update the UI to always use the centralized "current user location" StateFlow.
- Optionally, display the source (device/phone) and staleness warnings.

#### E. Remove/Guard Phone-to-Device Location Updates
- In `sendLocationUpdate` and similar methods, only send the phone's location to the device if the device is explicitly configured to use the phone's GPS.
- Otherwise, suppress these updates to avoid confusion.

#### F. Code Comments and Documentation
- Clearly document the logic and rationale in code comments, especially around the fallback and preference logic.

### 4. Implementation Steps
1. Audit: Find all usages of user location in the app (UI, ViewModels, services). Identify where the current user location is set and exposed.
2. Centralize State: Create or update a `StateFlow<LatLng?>` (or similar) in `MeshNetworkService` for the user's current location.
3. Update Protocols: In both `MeshNetworkProtocol` and `MeshtasticBluetoothProtocol`, update this StateFlow when a new position is received for the user's own node. Add logic to determine if a position packet is for the user's own node (compare node IDs).
4. Fallback Logic: Add a timer or staleness check to fallback to phone GPS if no device update is received in a reasonable time.
5. UI Update: Refactor UI/ViewModels to observe only the centralized StateFlow for the user's location.
6. Suppress Unnecessary Updates: Guard `sendLocationUpdate` and similar methods to only send phone location if the device is configured for it.
7. Testing: Test with both device types, with and without device connected, and with/without GPS fix.
8. Documentation: Add code comments and update any developer documentation.

### 5. Example Pseudocode
```kotlin
// In MeshNetworkService
private val _userLocation = MutableStateFlow<LatLng?>(null)
val userLocation: StateFlow<LatLng?> = _userLocation

// In MeshtasticBluetoothProtocol.handleIncomingPacket
if (meshPacket.from == myNodeId && meshPacket.hasDecoded() && isPositionPacket(meshPacket)) {
    val position = parsePosition(meshPacket)
    _userLocation.value = position
    lastDeviceLocationTimestamp = System.currentTimeMillis()
}

// Fallback logic (e.g., in a coroutine)
if (System.currentTimeMillis() - lastDeviceLocationTimestamp > STALE_THRESHOLD) {
    _userLocation.value = getPhoneLocation()
}
```

### 6. Consistency with Layer 2
- Ensure the same logic and fallback is used for both protocol types.
- The UI and app logic should not need to know which protocol is active; they just observe the centralized user location.

### 7. Optional: Advanced User Settings
- Allow advanced users to override the preference (e.g., always use phone, always use device, or auto).
- Expose the source and staleness in the UI for transparency.
